# A Vue3 UI library template modify
...
// 自定义resolver解析组件
function UiResolver(): ComponentResolver {
  return {
    type: "component",
    resolve: (name: string) => {
      // console.log(name);
      if (name.match(/^(Lib[A-Z]|lib-[a-z])/))
        return {
          name,
          from: `my-library`,
          sideEffects: `my-library/dist/es/${name}/style`, // 自动使用样式
        };
    },
  };
}
...
- [ ] Replace all `my-library` words with your libary name. Just search `my-library` and replace them in VScode
  - `.gitignore`
  - `package.json`
  - `vite.config.ts`
  - `.vitepress`
